      ...
      <div className={`rounded-[24px] max-w-lg w-full overflow-hidden shadow-xl border animate-in zoom-in-95 duration-200 ${isDark ? 'bg-[#22211e] border-neutral-800/80 text-[#f2efe9]' : 'bg-white border-slate-200 text-slate-800'}`}>
        ...
        <form onSubmit={onSubmit} className="p-5 space-y-4">
          ...
          <div className={`pt-3 border-t flex items-center justify-end space-x-2.5 ${isDark ? 'border-neutral-850' : 'border-slate-100'}`}>
            ...
            <button type="submit" disabled={isActionDisabled} className={`px-5 py-2.5 text-white font-extrabold text-xs rounded-xl shadow-md transition-all active:scale-95 flex items-center justify-center space-x-1.5 cursor-pointer ${isSubmitting ? 'bg-neutral-800 text-neutral-500 cursor-not-allowed opacity-60' : 'bg-red-600 hover:bg-red-700'}`}>
