...
import { Loader2, AlertTriangle, ShieldCheck, User, MessageSquare, ChevronDown, ChevronUp, Scale, CheckCircle, XCircle } from 'lucide-react';
...
export default function AdminReports() {
  ...
  const handleResolveSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        success('Report Resolved', 'Both parties have been notified.');
  ...
  const handleResolveEscalation = async (id: string, approve: boolean) => {
    const note = prompt(approve ? 'Optional admin note for approval:' : 'Reason for rejection (required):');
    if (!approve && !note?.trim()) return;
    try {
      const res = await apiResolveEscalatedCancellation(id, approve, note || undefined);
      if (res.success) {
        success(approve ? 'Cancellation Approved' : 'Cancellation Rejected', 'Both parties have been notified.');
        fetchEscalations();
      } else {
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${
            activeTab === 'reports'
              ? isDark ? 'bg-red-950/30 border-red-700/40 text-red-400' : 'bg-red-50 border-red-200 text-red-600'
                          ...
                          <div className={`p-4 space-y-3 max-h-60 overflow-y-auto border-t text-xs ${isDark ? 'bg-[#1c1b18] border-neutral-800' : 'bg-white border-slate-200'}`}>
                            ...
                            {!item.booking.messages?.length ? (
                              ...
                              item.booking.messages.map((msg) => {
                                ...
                                return (
                                  <div key={msg.id} className={`flex flex-col max-w-[85%] rounded-2xl p-3 border ${isReporter ? 'mr-auto bg-blue-500/5 border-blue-500/10' : 'ml-auto bg-neutral-800/20 border-neutral-800'}`}>
        ...
        <div className="space-y-4">
          {escalationsLoading ? (
            ...
            escalations.map(item => (
              <div key={item.id} className={`rounded-2xl p-5 border ${isDark ? 'bg-[#22211e] border-neutral-800/80' : 'bg-white border-slate-200'}`}>
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div>
                    <h5 className={`font-bold text-sm ${isDark ? 'text-[#f2efe9]' : 'text-slate-800'}`}>
                      {item.booking?.service?.title || 'Unknown Service'}
                    </h5>
                    <p className={`text-[10px] mt-0.5 ${isDark ? 'text-[#9a9690]' : 'text-slate-500'}`}>
                      Escalated {new Date(item.createdAt).toLocaleDateString()} · ID: {item.id.slice(0, 8)}
                    </p>
                  </div>
                  <span className="px-2.5 py-1 text-[10px] font-bold rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/20">
                    🚨 Escalated to Admin
                  </span>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className={`p-3 rounded-xl border text-xs ${isDark ? 'bg-neutral-800/30 border-neutral-700' : 'bg-slate-50 border-slate-200'}`}>
                    <p className="text-[10px] text-slate-400 font-bold mb-0.5">SEEKER</p>
                    <p className={`font-semibold ${isDark ? 'text-[#f2efe9]' : 'text-slate-700'}`}>{item.booking?.seeker?.name}</p>
                    <p className="text-[10px] text-slate-400">Trust: {item.booking?.seeker?.trustScore}</p>
                  </div>
                  <div className={`p-3 rounded-xl border text-xs ${isDark ? 'bg-neutral-800/30 border-neutral-700' : 'bg-slate-50 border-slate-200'}`}>
                    <p className="text-[10px] text-slate-400 font-bold mb-0.5">PROVIDER</p>
                    <p className={`font-semibold ${isDark ? 'text-[#f2efe9]' : 'text-slate-700'}`}>{item.booking?.provider?.name}</p>
                    <p className="text-[10px] text-slate-400">Trust: {item.booking?.provider?.trustScore}</p>
                  </div>
                </div>

                {item.reason && (
                  <div className={`p-3 rounded-xl border mb-4 text-xs ${isDark ? 'bg-neutral-800/20 border-neutral-700 text-[#d4cfc7]' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                    <span className="font-bold text-[10px] text-slate-400 block mb-1">SEEKER'S REASON</span>
                    {item.reason}
                  </div>
                )}
                {item.providerNote && (
                  <div className={`p-3 rounded-xl border mb-4 text-xs ${isDark ? 'bg-neutral-800/20 border-neutral-700 text-[#d4cfc7]' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                    <span className="font-bold text-[10px] text-slate-400 block mb-1">PROVIDER'S RESPONSE</span>
                    {item.providerNote}
                  </div>
                )}
                ...
                <div className="flex gap-2 justify-end">
                  <button
                    onClick={() => handleResolveEscalation(item.id, false)}
                    className="px-3.5 py-2 text-[10px] font-bold rounded-lg bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-all flex items-center gap-1.5"
                  >
                    <XCircle size={13} /> Deny Cancellation
                  </button>
                  <button
                    onClick={() => handleResolveEscalation(item.id, true)}
                    className="px-3.5 py-2 text-[10px] font-bold rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all flex items-center gap-1.5"
                  >
